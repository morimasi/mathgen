...
import { generatePositionalConceptsProblem } from '../services/readinessService';
...
const PositionalConceptsModule: React.FC<ModuleProps> = ({ onGenerate, setIsLoading, contentRef, autoRefreshTrigger, lastGeneratorModule }) => {
    ...
    const handleGenerate = useCallback(async (clearPrevious: boolean) => {
        ...
        try {
            ...
            const results = Array.from({ length: problemCount }, () => generatePositionalConceptsProblem(settings));
            const problems = results.map(r => r.problem);
            const title = results.length > 0 ? results[0].title : '';

            onGenerate(problems, clearPrevious, title, 'positional-concepts', settings.pageCount);
        } catch (error: any) {
            alert(error.message);
            console.error(error);
        }
    ...
    return (
        <div className="space-y-4">
            <h2 className="text-lg font-semibold">Konum ve Yön Kavramları</h2>
            <Select
                label="Kavram Türü"
                id="positional-type"
                value={settings.type}
            ...
            />
            <NumberInput label="Nesne Sayısı" id="item-count-pos" min={2} max={5} value={settings.itemCount} onChange={e => handleSettingChange('itemCount', parseInt(e.target.value))} />
