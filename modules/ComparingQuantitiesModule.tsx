...
import { generateComparingQuantitiesProblem } from '../services/readinessService';
...
const ComparingQuantitiesModule: React.FC<ModuleProps> = ({ onGenerate, setIsLoading, contentRef, autoRefreshTrigger, lastGeneratorModule }) => {
    ...
    const handleGenerate = useCallback(async (clearPrevious: boolean) => {
        ...
        try {
            ...
            const results = Array.from({ length: problemCount }, () => generateComparingQuantitiesProblem(settings));
            const problems = results.map(r => r.problem);
            const title = results.length > 0 ? results[0].title : '';

            onGenerate(problems, clearPrevious, title, 'comparing-quantities', settings.pageCount);
        } catch (error: any) {
            alert(error.message);
            console.error(error);
        }
    ...
    return (
        <div className="space-y-4">
            <h2 className="text-lg font-semibold">Miktarları Karşılaştırma</h2>
            <Select
                ...
                options={[
                    { value: ComparisonType.MoreLess, label: 'Daha Fazla / Daha Az' },
                    { value: ComparisonType.BiggerSmaller, label: 'Daha Büyük / Daha Küçük' },
                    { value: ComparisonType.TallerShorter, label: 'Daha Uzun / Daha Kısa' },
                ]}
            />
            <Select
                label="Tema"
            ...
            />
            <NumberInput label="En Fazla Nesne Sayısı" id="max-object-count" min={3} max={20} value={settings.maxObjectCount} onChange={e => handleSettingChange('maxObjectCount', parseInt(e.target.value))} />
