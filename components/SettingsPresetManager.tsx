...
const SettingsPresetManager = <T,>({ moduleKey, currentSettings, onLoadSettings }: SettingsPresetManagerProps<T>) => {
    ...
    const handleSave = () => {
        ...
        if (name) {
            ...
            addToast(`'${name}' ayar seti kaydedildi.`, 'success');
            setNewPresetName('');
    ...
    const handleLoad = () => {
        if (selectedPreset) {
            ...
            if (loaded) {
                ...
                addToast(`'${selectedPreset}' ayar seti başarıyla yüklendi.`, 'success');
            }
        }
    };
    ...
    const handleDelete = () => {
        if (selectedPreset) {
            if (window.confirm(`'${selectedPreset}' adlı ayar setini silmek istediğinizden emin misiniz?`)) {
                ...
                addToast(`'${selectedPreset}' silindi.`, 'info');
            }
        }
    };
    ...
    const handleToggleFavorite = () => {
        if (selectedPreset) {
            ...
            const isFavorite = favorites.includes(selectedPreset);
            addToast(isFavorite ? `'${selectedPreset}' favorilerden kaldırıldı.` : `'${selectedPreset}' favorilere eklendi.`, 'info');
        }
    };
